...
from open_meeting.scraper import hardwick_town
...
class TestCreateDriver:
    ...
    def test_headless_adds_argument(self):
        with patch('open_meeting.scraper.hardwick_town.webdriver.Firefox'), \
             patch('open_meeting.scraper.hardwick_town.FirefoxOptions') as mock_cls:
...
class TestFetchMinutesList:
    ...
    def test_returns_parsed_records(self):
        ...
        with patch.object(hardwick_town, '_create_driver', return_value=ctx), \
             patch.object(hardwick_town, '_wait_past_cloudflare'), \
             patch('open_meeting.scraper.hardwick_town.time.sleep'):
